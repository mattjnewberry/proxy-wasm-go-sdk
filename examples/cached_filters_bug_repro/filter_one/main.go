...
func (ctx *helloWorld) OnPluginStart(pluginConfigurationSize int) types.OnPluginStartStatus {
	...
	proxywasm.LogInfo("Filter 1: OnPluginStart from Go!")
...
func (ctx *helloWorld) OnTick() {
	t := time.Now().UnixNano()
	proxywasm.LogInfof("Filter 1: It's %d: random value: %d", t, rand.Uint64())
	proxywasm.LogInfof("OnTick called in Filter 1")
}
